...
data = pd.read_csv(DATA_PATH.joinpath("AG_Discrete_DS06-Jul-2020.csv"))
...
excel = pd.read_csv(DATA_PATH.joinpath("QA.csv"))
...
layout = html.Div([
    dbc.Row(dbc.Col(html.H2("Kovaltry Drug Substance"),
...
@app.callback([Output('graph', 'figure'), Output('graph1', 'figure')],
             [Input('col', 'value')])
def cb(col):
    ...
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=list(data.dropna(subset=[col], how = 'any').Date),
                         y=list(data[col].dropna()),
                         name = col,
                         mode="lines+markers",
                         line = dict(color="black")
